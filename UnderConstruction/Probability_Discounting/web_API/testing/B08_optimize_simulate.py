...
#------------------------------------------------------------------------------
# generate paradigm B based on best param estimates
def generateParadigmAlt(pocc, r1s, pars):
...
def countProblemsAlt(parameters, task):
    # Trial Generation
    pars=parameters
    if task == "reward":      
        # Änderung Mathieu: 1 durch 50 ersetzen 
        ...
        r1s=[-5, -10, -20, -50]          # define uncertain rewards used for task B
    p_occ = np.array([.1, .25, .5, .75, .9])
    r1_B, r2_B, p_cert, odds_B, occp_B, flag = generateParadigmAlt(p_occ, r1s, pars)
...
params_test = [0.34, 0.3]
problems, outdata = countProblems(params_test, "loss")

problemsAlt, outdataAlt = countProblemsAlt(params_test, "loss")
